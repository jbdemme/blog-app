        ...
        <div className="max-w-3xl mx-auto p-6">
            ...
            <form className="grow flex gap-4 mb-12">
                <input
                    type="text"
                    name="q"
                    className="grow border border-white rounded-lg bg-gray-950 px-2"
                    data-testid="filter-input"
                />
                <button
                    type="submit"
                    className="bg-cyan-600 p-2 rounded-lg focus:shadow-outline hover:bg-cyan-800"
            ...
            <ul className="space-y-2" data-testid="blogs-list">
                {blogs.map(blog => (
                    <li key={blog.id} className="p-3 full-w hover:bg-mist-900">
                        ...
                        <span className="text-foreground/50">{blog.author} ({blog.url}) {blog.likes} likes</span>
                    </li>
                ))}
            </ul>
        </div>
